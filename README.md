...
默认模板已启用 `rss.feeds_file: "custom/rss_feeds.yaml"`，会加载仓库内维护的完整 RSS 源清单；同时通过 `rss.enabled_ids_file: "custom/rss_enabled_default.txt"` 仅默认启用一批稳定高质量源，其余源保留在清单里，按需打开即可。
